<?php
  ...
  $newsid = $_GET["id"];
  $newsdata = readSingleNews($newsid);
  $expdate = null;
  $expday = substr($newsdata["expire"], 8, 2);
  $expmonth = substr($newsdata["expire"], 5, 2);
  $expyear = substr($newsdata["expire"], 0, 4);
  $expdate = null;
  $photoinputerror = "";
  $inputerror = "";
  $notice = null;
  ...
  if(isset($_POST["newssubmit"])){
	...
	if(empty($inputerror) and empty($photoinputerror)){
		//uudis salvestada
		//echo $news;
		$result = modifyNews($newstitle, $news, $expdate, $newsid);
		if($result == 1){
			$notice = "Uudis on muudetud!";
			//$error = "";
			//$newstitle = "";
			//$news = "";
