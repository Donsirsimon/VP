<?php
	...
	function readPublicPhotoThumbsPage($privacy, $limit, $page = 1){
		...
		$skip = ($page - 1) * $limit;
		$conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
		//$stmt = $conn->prepare("SELECT filename, alttext FROM vpphoto WHERE privacy >= ? AND deleted IS NULL ORDER BY vpphotos_id DESC LIMIT ?");
		...
		while($stmt->fetch()){
			...
			//<img src="failinimi.laiend" alt="alternatiivtekst" class="thumbs" data-fn="failinim.laiend" data_id="7">
	...
	function readLatestPhoto($privacy){
		$photohtml = null;
		$conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
